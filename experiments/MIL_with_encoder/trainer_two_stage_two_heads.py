...
class TrainerTwoStageTwoHeads:
    ...
    def run_one_epoch(self, model, data_loader, epoch: int, train: bool = True):
        ...
        for data, bag_label, meta in tepoch:
            ...
            class_loss += loss_tumor.item()
            polar_loss += loss_polar.item()
            if not non_important_relevancy_probs.isnan().any():
                relevancy_loss += loss_relevancy_non.item()
            if not important_relevancy_probs.isnan().any():
                relevancy_loss += loss_relevancy_important.item()

            error = calculate_classification_error(bag_label.cpu(), prediction)
        ...
        print(
            f"Main classification loss: {round(class_loss, 3)}, non_rel loss: {round(relevancy_loss, 3)}, polar loss: {round(polar_loss, 3)}")
