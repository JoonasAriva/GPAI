...
from focusMIL import ContastiveLoss

depth_loss_fuction = DepthLossSamplePatches().cuda()
contrastive_loss_function = ContastiveLoss().cuda()
    ...
    def run_one_epoch(self, model, data_loader, epoch: int, train: bool = True, local_rank=0):
        ...
        for data_dict in tepoch:
            ...
            with torch.amp.autocast(device_type='cuda'), torch.no_grad() if not train else nullcontext():
                ...
                contra_loss = contrastive_loss_function(out["proj_vectors"], data_dict["bag_idx"], data_dict["label"],
                                                        out["compass_scores"], out["instance_scores"])

                (dloss, hloss, wloss), norm_loss = depth_loss_fuction(out["compass_scores"], data_dict["patch_centers"],
                                                                      patient_ids=data_dict["bag_idx"])
                depth_loss = (dloss + hloss + wloss + norm_loss)
                # print("KL:", KL_loss.item())
                # print("cls loss:", cls_loss.item())
                # print("depth loss:", depth_loss.item())
                # print("contra loss:", contra_loss.item())
                total_loss = (
                        0.001 * KL_loss
                        + 1 * cls_loss
                       # + 0.05 * depth_loss
                ...
                results["depth_loss"] += depth_loss.item()
                results["contra_loss"] += contra_loss.item()
                results["d_loss"] += dloss.item()
                results["h_loss"] += hloss.item()
                results["w_loss"] += wloss.item()
                results["norm_loss"] += norm_loss.item()
