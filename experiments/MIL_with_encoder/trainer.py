...
class Trainer:
    def __init__(self, optimizer, loss_function, attention_loss, crop_size: int = 120, nth_slice: int = 4,
                 ...
                 full_pass_for_each_slice=False, scheduler = None):
        ...
        self.full_pass = full_pass_for_each_slice
    ...
    def run_one_epoch(self, model, data_loader, epoch: int, train: bool = True):
        ...
        for data, bag_label, file_path in tepoch:
            ...
            with torch.cuda.amp.autocast(), torch.no_grad() if not train else nullcontext():
                ...
                if self.full_pass:
                    Y_prob, Y_hat, A, full_pass_probs = model.forward(data, return_unnorm_attention=True,
                                                                      full_pass=True)
                else:
                ...
                if self.full_pass:
                    attention_loss, terms = self.attention_loss(A, full_pass_probs)
                else:
                    pass
            ...
            #mean_l, max_l, min_l = terms

            #mean_loss += mean_l.item()
            #max_loss += max_l.item()
            #min_loss += min_l.item()
            #balance_loss += balance_l.item()
        ...
        print(
            f"Main classification loss: {round(class_loss, 3)}, attention loss: {round(att_loss, 3)} = (not scaled)(min: {round(min_loss, 3)}, max: {round(max_loss, 3)})")
