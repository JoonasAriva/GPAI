...
from dynamic_network_architectures.building_blocks.compass_head import COMPASSCoordinateHead
...
class PlainConvUNetCOMPASS(nn.Module):
    """
    PlainConvEncoder + COMPASS coordinate prediction head.

    Uses the same encoder as PlainConvUNet but replaces the UNet decoder with
    a lightweight coordinate regression head that only uses the bottleneck
    (last stage) output — no skip connections.

    Input:  (B, input_channels, *spatial)
    Output: (B, coord_dim) predicted 3D coordinates (D, H, W)
    """
    ...
    def __init__(
        ...
        self.compass_head = COMPASSCoordinateHead(
            in_channels=features_per_stage[-1],
            conv_op=conv_op,
            hidden_dim=compass_hidden_dim,
            coord_dim=coord_dim,
        )

    def forward(self, x):
        features = self.encoder(x)
        return self.compass_head(features)
...
if __name__ == "__main__":
    ...
    # --- COMPASS coordinate head test ---
    # 3D test: thin slices (1, 128, 128) mimicking COMPASS patching
    data = torch.rand((16, 1, 1, 128, 128))  # 16 patches, 1 channel
    compass_model = PlainConvUNetCOMPASS(
        input_channels=1,
        n_stages=6,
        features_per_stage=(32, 64, 128, 256, 320, 320),
        conv_op=nn.Conv3d,
    ...
    compass_model.initialize(compass_model)
    coords = compass_model(data)
    print(f"COMPASS 3D: input {data.shape} -> coords {coords.shape}")  # (16, 3)

    # 2D test: 128x128 patches with 3 channels (RGB from consecutive slices)
    data = torch.rand((16, 3, 128, 128))
    compass_model_2d = PlainConvUNetCOMPASS(
        input_channels=3,
        n_stages=6,
        features_per_stage=(32, 64, 128, 256, 320, 320),
        conv_op=nn.Conv2d,
    ...
    compass_model_2d.initialize(compass_model_2d)
    coords = compass_model_2d(data)
    print(f"COMPASS 2D: input {data.shape} -> coords {coords.shape}")  # (16, 3)
